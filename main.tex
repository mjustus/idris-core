...
\newcommand{\checkCtxJ}[3]{#1; #2 \ENT {#3}}
...
\newcommand{\NIL}{\EMPTY}
\newcommand{\CONS}[3]{{#2} \mathpunct{\prescript{#1}{}{,}} {#3}}
...
and environments
$$
\delta, \phi \mathrel{\text{::=}} \NIL \mid \CONS q t \delta
$$
as a syntactic convenience for defining data types. These give rise to telescope abstraction $\Delta \to B$
\begin{align*}
  {\EMPTY \to B} &= B\\
  {(\name x \OFq q A), \Delta \to B} &= \PiT x q A {(\Delta \to B)}
\end{align*}
...
\subsubsection{Stuck function calls}

Function definition by pattern matching requires maintaining what amounts to a suspended substitution on function applications that do not yet match a particular clause. For example, the open term \idrisinline{f 5 n}
\begin{code}
  f : Nat -> Nat -> Nat
  f m Z = b_Z
  f m n@(S n') = b_S
  ...
  g m n =
    case n of
      ...
      S n' => b_S
\end{code}
in context \idrisinline{(n : Nat)} is stuck whereas the application \idrisinline {g 5 n} using the corresponding function defined by case split reduces immediately to its body
\begin{code}
  case n of
    Z => b_Z [5/m]
    S n' => b_S [5/m]
\end{code}
resulting in a stronger equational theory.

Ideally, we would want the equational strength of the latter combined with the expressiveness afforded by with-abstraction provided by the former.
...
\inferrule*[Right=Def-DataCon] {
  \left( \definition{tc} \OF {\Delta \to \Type} \right) \in \Sigma\\\\
  \checkCtxJ \Sigma \EMPTY \Phi\\
  \checkJ \Sigma \Phi \delta \zero \Delta
} {
...
\subsection{Eta-expansion for records}

The core type theory of Idris does not have records. Instead, records are desugared into single constructor data types. So for the purpose of this note we will refer to the latter as records. Although currently not implemented, strengthening conversion checking by performing eta-expansion on records is in the planning.

Unfortunately, naively adding an eta-rule for records to the type theory clashes with linearity checking. Consider a record
\begin{code}
record R : Type
  constructor MkR
  1 x : Int
  1 y : Int
\end{code}
and an instance of the usual eta-law on a linear record variable $(\name r \OFq \one \texttt{R})$ at ambient quantity $\one$
\begin{mathpar}
\inferrule* {
  (\name r \OFq \one \texttt{R}) \in \Gamma
} {
  \checkEqJ \Sigma \Gamma {\name r} {\App \one {\App \one {\texttt{MkR}} {{\name r}.\texttt{x}}} {{\name r}.\texttt{y}} } \one \texttt{R}
}
\end{mathpar}
Going from left to right, it is no longer immediately obvious that $\name r$ is used linearly without also checking whether each linear field\footnote{The types of Idris' generated field projection functions, e.g. ${.\texttt{x}} \OF {\PiT r \any {\texttt{R}} \texttt{Int}}$, already rule out application to linear variables or compound expressions that make use of linear variables but that is beside the point.} \texttt{R} has been used precisely once.

This should not stop us, however, from introducing eta-expansion when no linear variables are involved. The following rule
\begin{mathpar}
\inferrule* {
  \checkJ \Sigma \Gamma e {\multQ \any p} \texttt{R}
} {
  \checkEqJ \Sigma \Gamma e {\App \one {\App \one {\texttt{MkR}} {e.{\texttt{x}}}} {e.{\texttt{y}}} } p \texttt{R}
}
\end{mathpar}
corresponds to the usual eta-law but rules out any linear variables being present in $e$.
...
\inferrule*[Right=Lin-Case]{
  ...
  \forall i, j.~ (u_i \smallsetminus \Phi_i) = (u_j \smallsetminus \Phi_j)\\\\
  \presence p = \one \land q = \zero \implies i \leq 1
} {
